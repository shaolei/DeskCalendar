...
本轮修复后，**B1/S2/S1/S3/S4 已闭环**（S1 单写者收口主循环；S3 抢前台 + activated 守卫消除「闪一下就没了」；S4 字体重构后单次 Render 仅解析一次 ~20MB `msyh.ttc`），S6 作为烟测副产物被识别并修复。当前代码可进入 Windows 真机手测；发版前剩余 S5/N1/N2/N3 待处理。
