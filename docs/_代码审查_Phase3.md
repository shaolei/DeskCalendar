...
**结论**：降级路线正确、架构干净，**修掉 B1 + S2 即可视为可发布到真机烟测**；S1（并发）建议发布前改为单写者设计。
...
### S3　窗口激活/失焦：显示后可能立即被 `WM_ACTIVATE WA_INACTIVE` 自隐藏
...
**Suggestion**：`wmUserShow` 处理里 `ShowWindow` 后补 `SetForegroundWindow(hwnd)`（跨进程需 `AllowSetForegroundWindow` 由托盘进程放行，或先 `ShowWindow(SW_SHOWNOACTIVATE)` 再 `SetForegroundWindow`）；更稳妥是加 `activated bool` 标志，仅当窗口确实激活过、后续失焦才隐藏。此条**必须在真机（Windows）烟测验证**，属于 ADR-08 行动项 #5 范围。
...
3. **🟡 S1** —— 共享 Config/Calendar 写收口主循环（或加锁）。零 CGO 下这是唯一的并发正确性兜底。
4. **🟡 S3 / S5** —— 随真机烟测一并验证与修补（激活/失焦、GDI 对象生命周期）。
...
- **S1**：Config/Calendar 并发写收口主循环（单写者）或加锁；零 CGO 无法 `-race`，需设计层面兜底。
- **S3**：`wmUserShow` 后补 `SetForegroundWindow` 防止 WA_INACTIVE 自隐藏；需真机有显示器/任务栏交互时验证。
...
本轮修复后，**B1/S2 在真机（Windows）烟测中确认闭环**，S6 作为烟测副产物被识别并修复。当前代码可进入 Windows 真机手测；发版前建议优先处理 S1。
