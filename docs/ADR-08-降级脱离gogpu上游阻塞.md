...
- **v1.2**：显隐动画（若产品认为必要，走 layered+alpha 或 DWM 过渡，自绘 `Animator` 已在 Animation.md 设计且无 gogpu 依赖）。→ **已落地**：GitHub 里程碑 `v1.2 动画` + epic **#150**（2026-07-14 创建）。
...
## GitHub 里程碑落地映射（2026-07-14 重排，与本文决策一致）

> 本文 §6 原路线图（v1.0~v1.4+）已按用户决策在 GitHub 上重排落地。所有里程碑**仅含 epic、不拆子 issue**（用户要求降低 issue 管理复杂度）。

| GitHub 里程碑 | 对应本文项 | 主要 Epic / Issue |
|---|---|---|
| `v1.0 (MVP)` — 已打 tag `v1.0.0` | v1.0 核心 | 收口提交 `d183553`；120 个任务 issue 已 closed |
| `v1.1 待办事项与天气` | v1.1 圆角 + 原 60-Todo(v1.1)/70-Weather(v1.2，按决策并入) | #147 圆角(DWM)、#148 待办、#149 天气、#41 圆角DPI |
| `v1.2 动画` | v1.2 显隐动画 | #150 显隐过渡（淡入+上移） |
| `v1.3 主题换肤` | v1.3 复杂设置页（皮肤/字体/周起始日等归主题） | #119 设置页后备、#55 多屏记忆、#39 换肤联动圆角 |
| `v1.4 插件系统` | v1.4+ 插件只读可见性 | #49 / #40 / #23 / #7（全插件类） |
| `v1.5 自动更新` | v1.4+ 自更新 | #8 自更新重启装配 |

**偏离本文原路线图之处（均为用户主动决策，不回退 ADR-08 核心降级决策）**：
- 原 v1.1 仅"圆角"；现 v1.1 扩容为"待办事项 + 天气 + 圆角"（待办/天气为文档既有规划，提前并入 v1.1，非新增需求）。
- 原 v1.3「复杂设置页」现落实为「主题换肤」里程碑；设置项仍以托盘菜单为主，复杂设置页仅作后备（#119）。
- 原 v1.4+ 的「多屏/DPI 锚定」仍归 platform 层（#55 暂留 v1.3），未单列里程碑。
- 各里程碑 issue 编号已在 GitHub 实建，本文 §6 仅作决策层面路线图，具体以 GitHub 里程碑为准。
