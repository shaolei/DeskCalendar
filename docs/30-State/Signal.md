# Signal — gogpu/ui 响应式原语

> 版本：v1.0-draft ｜ 最后更新：2026-07-07 ｜ 模块：30-State ｜ 子主题：Signal

## 1. 📦 package 设计

- **包名**：`state`（状态容器统一归属 `internal/state`）。Signal 原语本体由 `gogpu/ui` 提供，本项目**直接复用、不二次封装**，保持零抽象损耗。
- **所在目录**：`internal/state`（Signal 的使用约定与组合样板在此收敛，见 `signal.go` / `doc.go`）。
- **职责一句话**：Signal 是 `gogpu/ui` 的泛型响应式原语，承载"值变化 → 通知订阅者 → 触发 UI 重绘"的最小闭环；它是本项目单向数据流的最终落点。
- **依赖方向**：
  - 依赖：`gogpu/ui`（Signal 本体）、`runtime`（主线程绑定）。
  - 被依赖：`internal/ui`（视图订阅 Signal 渲染）、`internal/shell`（主线程 OnUpdate 消费）、`internal/state` 的 Store（内部持有 Signal）、各 feature（读写领域状态）。
- **对外暴露的公开符号**：`ui.NewSignal[T]`、`ui.Signal[T]` 接口（Get/Set/Update/Subscribe/Observe）；本项目约定的"主线程唯一写入"规则（见 §6）。
- **边界**：
  - 归它管：值的读写、变化通知、订阅生命周期、与 OnUpdate 重绘的衔接。
...
classDiagram
    ...
    class MainThread {
        <<desktop.Run + OnUpdate>>
        +LockOSThread()
        +RequestRedraw()
    }

    Signal~T~ <.. Subscriber : Observe/Subscribe
    Signal~T~ ..> MainThread : Set/Update 仅在此触发重绘
    Note for Signal~T~ "ui.NewSignal[T](initial) 创建实例"
```
...
要点：生产者**永不直接调用 `Signal.Set`**，只通过命令通道把意图交给主线程；主线程 `OnUpdate` 是唯一能改变 Signal 值的地方（见 §6 线程安全铁律）。
...
sequenceDiagram
    ...
    Note over Main: runtime.LockOSThread() 已绑定主线程
    Prod->>Ch: Enqueue(CmdSelectDate{...})
    Main->>Ch: 轮询取出 Cmd
    Main->>Sig: Set(newValue)  ← 唯一合法写入点
    Sig-->>View: Observe(old,new) 回调
    Sig->>Main: 标记脏区 + RequestRedraw()
    Main->>View: 下一帧重绘
```

**线程安全铁律**：
- `Signal.Set` / `Signal.Update` **只能在主线程调用**（即 `desktop.Run` 所在、已 `runtime.LockOSThread` 的线程）。因为渲染与 Signal 通知都在主线程帧循环内发生，内部无需 `sync.Mutex`，避免锁竞争与跨线程撕裂读。
...
- 与 UI 重绘关系：`Set` 后 `gogpu/ui` 标记对应视图脏区并请求重绘；若主循环处于空闲态，调用 `gogpuApp.RequestRedraw()`（非阻塞）唤醒下一帧，避免 busy loop。
- 订阅回调（`Subscribe`/`Observe` 的 `fn`）一律在主线程派发，回调内可安全读取/修改其他 Signal（仍仅主线程），**不可在回调中做阻塞 IO**。

## 7. 🔌 Plugin API

**N/A**。Signal 是 UI 框架层（`gogpu/ui`）的底层响应式原语，属于"状态引擎内部件"，不应向插件暴露。插件不直接订阅或写入 Signal——它们通过 feature 层定义的领域事件 / 钩子（见各 feature 模块与 `80-Plugin`）与系统交互，由 feature 内部translate 为命令并最终落到 Signal。直接暴露 Signal 会让插件获得跨主线程写入能力，违背 §6 线程安全铁律，故不开放。
...
stateDiagram-v2
    ...
    空闲 --> 收到命令: OnUpdate 取出 Cmd
    收到命令 --> 写入Signal: 主线程 Set/Update
    写入Signal --> 通知订阅: Observe/Subscribe 触发
    通知订阅 --> 重绘: RequestRedraw + 帧渲染
    重绘 --> 空闲: 脏区清空前稳定
    空闲 --> [*]: 进程退出
```
...
以下为 `gogpu/ui` 的 Signal 契约示意（泛型 `Signal[T]`），本项目直接复用，可编译风格：

```go
package ui // 实际来自 gogpu/ui；本项目在 internal/state 组合使用，不重新定义

// Signal 是 gogpu/ui 的泛型响应式原语。
// 线程约束：Set/Update 仅可在主线程调用；Get 可任意线程只读；
// Subscribe/Observe 注册在主线程，回调也在主线程派发。
...
| 版本 | 任务 | 验收标准 |
|------|------|----------|
| v1.0 (MVP) | 接入 `gogpu/ui` Signal 原语；确立"主线程唯一写入"规则与 channel 桥接约定 | 视图订阅 Signal 后，主线程 Set 能触发重绘；systray 线程直接 Set 在 code review 中被禁止（静态约定） |
