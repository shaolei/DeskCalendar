...
type Options struct {
	...
	Icon []byte
	// Config/ConfigPath 退出前持久化的配置与路径。
...
//
// 启动顺序（路径 D）：
//  1. 构造窗口（内部启动窗口线程消息泵）、托盘管理器。
//  2. 接线生命周期（anchor=托盘矩形；persist=写 config；quit=取消 ctx）。
//  3. 设置托盘图标/提示，注册左键单击 → CmdToggle。
//  4. go tray.Run(ctx, cmdCh)（托盘消息泵，独立 goroutine）。
//  5. 主循环 select cmdCh：消费命令 → lifecycle.Handle；StateQuit → 清理并返回。
